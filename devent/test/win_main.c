...
static void callAccept(HANDLE port,SOCKET listener, struct sockaddr* local) {
	...
	DWORD addressLength = sizeof(*local) + 16;
	...
	BOOL r = lpfnAcceptEx(listener, acceptSocket, io->lpOutputBuffer, 0, addressLength, addressLength, &dwBytes, io);
...
//	setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, io->local, io->local_len);
	...
	if (!lpfnConnectex(fd, (const struct sockaddr *) &address, sizeof(address), NULL, 0, NULL, io) && WSAGetLastError() != WSA_IO_PENDING) {
...
int main(int argc, char** args) {
...

	{
		...
		CreateIoCompletionPort(fd, cp, (ULONG_PTR)fd, 0);

		callAccept(cp, fd, (struct sockaddr*)&address);
	}
