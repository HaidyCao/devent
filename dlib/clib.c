...
char *sockaddr_to_string(struct sockaddr *addr, char *s, int size) {
  ...
  if (addr->sa_family == AF_INET) {
    ...
    if (inet_ntop(in->sin_family, &in->sin_addr, ip, sizeof(ip)) != NULL) {
      if (s && size >= strlen(ip)) {
        sprintf(s, "%s:%d", ip, (int) htons(in->sin_port));
        return s;
      } else {
        sprintf(ip, "%s:%d", ip, (int) htons(in->sin_port));
        return ip;
      }
    }
    ...
    if (inet_ntop(in->sin6_family, &in->sin6_addr, ip, sizeof(ip)) != NULL) {
      if (s && size >= strlen(ip)) {
        sprintf(s, "%s:%d", ip, (int) htons(in->sin6_port));
        return s;
      } else {
        sprintf(ip, "%s:%d", ip, (int) htons(in->sin6_port));
        return ip;
      }
    }
  }
...
int parse_address(const char *addr_str, struct sockaddr *addr, socklen_t *len) {
  ...
  if (*addr_str == '[') {
    ...
    unsigned long ipv4 = inet_addr(domain);
    free(domain);
    if (ipv4 != INADDR_NONE) {
      port_str++;
...
int port_to_int(char p[2]) {
  int a = (((int) p[0] << 8) & 0xFF00) | ((int) p[1] & 0xFF);
  return 0x0000FFFF & a;
}
